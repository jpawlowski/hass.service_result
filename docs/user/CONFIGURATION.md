...
This document describes all configuration options and settings available in the Service Result Entities custom integration.
...
### `service_result.example_service`
...
```yaml
service: service_result.example_service
target:
  ...
  - alias: "Call service at sunset"
    trigger:
      - trigger: sun
        event: sunset
    action:
      - action: service_result.example_service
        target:
          entity_id: switch.device_name_switch
        data:
          parameter: "mode"
...
```yaml
blueprint:
  name: Service Result Entities Alert
...
- [GitHub Issues](https://github.com/jpawlowski/hass.service_result/issues) - Report problems

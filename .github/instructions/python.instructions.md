...
- Files: `snake_case.py`
- Classes: `PascalCase` prefixed with `ServiceResultEntities`
...
- Inherit from both platform entity and `ServiceResultEntitiesEntity` (order matters)
...
from custom_components.service_result.sensor import async_setup_entry
...
**Config entry data:** `entry_data: ServiceResultEntitiesData = hass.data[DOMAIN][entry.entry_id]`
